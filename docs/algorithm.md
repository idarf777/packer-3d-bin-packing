...
1. 全Boxの各軸寸法をレイヤー厚さの候補とする（回転モードによって軸を制限）
   - `"all"`: 幅・高さ・奥行の3軸すべてを候補にする
   - `"yAxis"`: 高さ軸のみを候補にする（幅・奥行はスキップ）
   - `"none"`: 高さ軸のみを候補にする
2. 各候補に対して**評価値**を計算する：
...
2. **`find_box()`**: その空きスペースに最適なBoxを検索
   - 各未パック Box について `analyze_box()` で回転パターンを試行
   - `"yAxis"` モードのBoxは最適向き（幅と高さを入替えた向き）を先に試行してから元の向きを試行
   - `"all"` モードのBoxは全6向きを試行（正方体の場合は1向きのみ）
   - `"none"` モードのBoxは元の向きのみ試行
...
**正方体パレットの最適化:** パレットの幅・高さ・奥行がすべて同じ値の場合、回転しても結果が変わらないため向き1のみを試行する。
...
**無効化オプション:** `Packer` コンストラクタの第3引数に `{ isNotUseBeamSearch: true }` を渡すと、ビームサーチを無効化して常に標準アルゴリズムを使用する。
...
**アルゴリズムへの影響:**
- `construct_layers()`: `"all"` は幅・高さ・奥行の3軸、`"yAxis"` と `"none"` は高さ軸のみを候補レイヤー厚さとして使用
- `find_box()` → `analyze_box()`: `"yAxis"` は最適向き優先＋元向きの2パターン、`"none"` は1向きのみ、`"all"` は最大6向きを試行
