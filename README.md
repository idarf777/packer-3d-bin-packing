...
物理的に安定した梱包を実現するための新機能です。安定モードが有効な場合、箱の底面(Y=0)以外では、上に配置される商品のX-Z面が下の商品のX-Z面から「はみ出す」ことを防止します。

### 特徴

- **物理的安定性の保証**: 上の商品が下の商品の境界内に完全に収まる
- **Y軸回転モード自動適用**: 安定モード有効時は自動的にY軸回転限定になる
- **厳密な支持面積計算**: 商品の90%以上が下位商品によって支えられる必要がある
...
1. **Y軸回転モード必須**: 安定モードは必ずY軸回転限定モードと併用される
