...
class Boxologic {
    ...
    simulatePlacementByHeightGroups() {
        ...
        for (var hi = 0; hi < sortedHeights.length; hi++) {
            ...
            if (this.remain_layout_height < groupHeight)
                continue;
    ...
    simulateLayerPackingForGroup(groupIndices) {
        ...
        while (currentX < this.pallet.layout_width && currentZ < this.remain_layout_length) {
            ...
            packedInLayer++;
            stablePlacements++;
            if (bestFit.length > rowMaxLength)
                rowMaxLength = bestFit.length;
    ...
    simulatePlacementWithHeightmap() {
        ...
        for (var iter = 0; iter < this.box_array.size(); iter++) {
            ...
            for (var pi = 0; pi < candidateXZ.length; pi++) {
                ...
                if (cx >= this.pallet.layout_width || cz >= this.pallet.layout_length)
                    continue;
                for (var bi = 0; bi < this.box_array.size(); bi++) {
                    ...
                    for (var oi = 0; oi < orientations.length; oi++) {
                        ...
                        if (cx + w > this.pallet.layout_width)
                            continue;
                        if (cz + l > this.pallet.layout_length)
                            continue;
                        ...
                        // Check height bound
                        if (baseY + h > this.pallet.layout_height)
                            continue;
                        // Stability check for non-ground placements
                        if (baseY > 0.01) {
                            if (!this.check_stability(cx, cz, w, l, baseY))
                                continue;
                        }
                        // Overlap check (safety net)
                        if (this.overlapsAnyPacked(cx, baseY, cz, w, h, l))
                            continue;
            ...
            for (var i = 0; i < this.box_array.size(); i++) {
                ...
                for (var pi = 0; pi < positions.length && !placed; pi++) {
                    for (var oi = 0; oi < orientations.length && !placed; oi++) {
                        ...
                        if (x + orient.width > this.pallet.layout_width)
                            continue;
                        if (y + orient.height > this.pallet.layout_height)
                            continue;
                        if (z + orient.length > this.pallet.layout_length)
                            continue;
                        // Check stability (support ratio >= 70%)
                        if (!this.check_stability(x, z, orient.width, orient.length, y))
                            continue;
                        // Check no overlap with existing packed boxes
                        if (this.overlapsAnyPacked(x, y, z, orient.width, orient.height, orient.length))
                            continue;
                        // Place the box
                        box.cox = x;
                        box.coy = y;
...
class GAWrapperArray extends packer.WrapperArray {
    ...
    constructResult() {
        ...
        var groupInstances = new tstl_1.default.HashMap(); // wrapperName -> [{instance, order}]
        for (var i = 0; i < this.size(); i++) {
            var wrapper = this.at(i);
